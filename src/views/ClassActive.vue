...
computed: {
  ...
  methods: {
    handleFileUpload(event) {
      this.foto_kehadiran = event.target.files[0];
    }, 
   
    storeDataAbsensi() {
      if (!this.foto_kehadiran) {
        Swal.fire({
          icon: 'warning',
      ...
      formData.append('foto_kehadiran', this.foto_kehadiran);
      ...
      const config = {
        method: 'post',
        url: import.meta.env.VITE_APP_BACKEND_URL_API + '/absensi',
        headers: {
      ...
      axios(config)
        ...
        .catch((error) => {
          console.error("Absensi Class Error:", error);
    ...
    showModalClockIn(kelasItem) {
  this.selectedClassId = kelasItem.kelas.id;
  this.nama = kelasItem.nama || 'Peserta';
  this.showModal.clockIn = true;
},

    closeModalClockIn() {
      this.showModal.clockIn = false;
    },
//     deleteAbsensi(kelas) {
//   Swal.fire({
//     title: 'Hapus Absensi?',
//     text: 'Kamu bisa Clock In ulang setelah ini.',
//     icon: 'warning',
...
//       axios.delete(`${import.meta.env.VITE_APP_BACKEND_URL_API}/absensi/${kelas.absensi_id}`)
//         .then(() => {
//           Swal.fire('Dihapus!', 'Absensi kamu berhasil dihapus.', 'success');
//           this.fetchTransactions(); // refresh list
//         })
//         .catch(error => {
//           console.error(error);
//           Swal.fire('Gagal', 'Tidak bisa menghapus absensi.', 'error');
//         });
//     }
//   });
// }
...
<div class="p-8 md:mx-32 mx-5 bg-white rounded-lg mb-8 animate-fade-in">
  <p class="mb-5 font-bold text-ungutext">Class Active</p>
  <hr class="bg-pinktua h-0.5 mt-1 border-none">
...
<div v-else-if="transactions.length === 0" class="text-center py-8 text-gray-500 font-medium">
  Anda belum memiliki class active.
</div>

  <div v-for="(kelas, index) in activeTransactions"
  ...
  <div class="flex flex-col justify-end items-end w-full md:w-auto mt-4 md:mt-0 mb-5 mr-8">
    <div class="flex space-x-2 mt-2 justify-center md:justify-end">
      <!-- <button class="bg-pinktua text-white px-3 py-1 rounded-lg hover:bg-gray-600 text-sm"  @click="showModalClockIn(kelas)">Clock In</button> -->
      
<!-- Tombol Clock In -->
<!-- Jika BELUM ada absensi -->
<button
  v-if="!kelas.absensi"
  @click="showModalClockIn(kelas)"
...
<!-- Jika absensi masih pending → munculkan tombol hapus -->
<!-- <button
  v-if="kelas.absensi"
  :disabled="kelas.absensi.status !== 'hadir'"
  @click="deleteAbsensi(kelas.absensi.id)"
  class="ml-2 px-3 py-1 rounded-lg text-white"
  :class="kelas.absensi.status === 'pending' ? 'bg-red-600 hover:bg-red-800' : 'bg-gray-400 cursor-not-allowed'"
>
  Hapus Absensi ({{ kelas.absensi.status }})
</button> -->
 ...
 <!-- pop up Clock In -->
    <!-- Modal Overlay -->
    <div v-if="showModal.clockIn" class="fixed inset-0 flex items-center justify-center z-50">
      <!-- Modal Content -->
      <div class="relative bg-ungutua bg-opacity-100 text-white rounded-lg p-6 w-full max-w-md shadow-lg z-60">
        <button @click="closeModalClockIn"
            class="absolute top-2 right-2 text-black hover:text-gray-200 text-xl font-bold">
      &times;
    </button>
    <form @submit.prevent="storeDataAbsensi">
      <h2 class="text-xl font-bold text-center mb-4 text-ungutext">{{ nama }}</h2>
        <div>
            <label class="block text-ungutext">Upload Photos</label>
            <input type="file" @change="handleFileUpload" class="w-full border border-gray-300 p-1 rounded-md text-ungutext bg-gray-200">
          </div>
